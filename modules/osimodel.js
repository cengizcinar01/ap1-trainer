...
const OSIModelView = (() => {
  ...
  const SCENARIOS = [
    { id: 'puzzle', title: 'Schichten-Puzzle', description: 'Bringe die 7 OSI-Schichten in die richtige Reihenfolge (Schicht 7 oben, Schicht 1 unten).' },
    { id: 'protocols', title: 'Protokoll-Zuordnung', description: 'Ordne die Netzwerk-Protokolle den korrekten OSI-Schichten zu.' },
    { id: 'hardware', title: 'Hardware-Zuordnung', description: 'Ordne Netzwerk-Geräte (Router, Switch, etc.) den korrekten OSI-Schichten zu.' }
  ];
  ...
  function renderExplanation(container) {
    ...
    container.querySelectorAll('.osi-layer').forEach(el => {
      el.querySelector('.osi-layer-header').addEventListener('click', () => el.classList.toggle('open'));
    });
  }
  ...
  function renderExerciseLayout(container) {
    ...
    else renderDndInner(exContent, HARDWARE);
  ...
  function setupDndLogic(container, isPuzzle) {
    ...
    const drops = container.querySelectorAll(isPuzzle ? '.osi-puzzle-slot' : '.osi-drop-zone');

    chips.forEach(chip => {
      chip.addEventListener('dragstart', (e) => { e.dataTransfer.setData('text/plain', chip.dataset.layer); e.dataTransfer.setData('source-id', chip.dataset.id || ''); chip.classList.add('dragging'); });
      chip.addEventListener('dragend', () => chip.classList.remove('dragging'));
      chip.addEventListener('click', () => {
        if (chip.parentElement.classList.contains('dz-content') || chip.parentElement.classList.contains('slot-content')) {
    ...
    drops.forEach(drop => {
      const target = drop.querySelector('.dz-content') || drop.querySelector('.slot-content');
      ...
      drop.addEventListener('drop', (e) => {
        ...
        const chip = id ? container.querySelector(`.pseudo-chip[data-id="${id}"]`) : container.querySelector(`.pseudo-chip[data-layer="${layer}"]`);
        if (chip) target.appendChild(chip);
      });
      drop.addEventListener('click', () => {
        if (window.selectedOsiChip) { target.appendChild(window.selectedOsiChip); window.selectedOsiChip.style.borderColor = ''; window.selectedOsiChip = null; }
      });
    });
  }

  function setupExerciseEvents(container, sc) {
    container.querySelector('#btnCheckExercise').addEventListener('click', () => {
      ...
      if (sc.id === 'puzzle') {
        ...
        container.querySelectorAll('.osi-puzzle-slot').forEach(slot => {
          ...
          if (chip) {
            ...
            if (chip.dataset.layer === slot.dataset.expected) { chip.classList.add('correct'); correct++; }
        ...
        chips.forEach(chip => {
          chip.classList.remove('correct', 'wrong');
          if (chip.dataset.layer === chip.closest('.osi-drop-zone').dataset.layer) { chip.classList.add('correct'); correct++; }
          else chip.classList.add('wrong');
        });
      }
    ...
    container.querySelector('#btnResetExercise').addEventListener('click', () => renderCurrentTab());
  }

  function cleanup() { cleanup_fns.forEach(fn => fn()); cleanup_fns = []; window.selectedOsiChip = null; }
