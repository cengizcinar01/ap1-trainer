const OSIView = (() => {
  ...
  const OSI_LAYERS = [
    { id: 7, name: 'Anwendung', desc: 'Schnittstelle zur Software', protocols: ['HTTP', 'FTP', 'SMTP', 'DNS', 'SSH'], pdu: 'Daten', hardware: 'Gateway', tcp: 'Anwendung' },
    { id: 6, name: 'Darstellung', desc: 'Formatierung & Verschlüsselung', protocols: ['JPEG', 'MPEG', 'ASCII', 'TLS/SSL'], pdu: 'Daten', hardware: 'Gateway', tcp: 'Anwendung' },
    { id: 5, name: 'Sitzung', desc: 'Dialogsteuerung', protocols: ['NetBIOS', 'RPC', 'SQL'], pdu: 'Daten', hardware: 'Gateway', tcp: 'Anwendung' },
    { id: 4, name: 'Transport', desc: 'End-zu-End-Kontrolle', protocols: ['TCP', 'UDP'], pdu: 'Segmente', hardware: 'L4-Switch', tcp: 'Transport' },
    { id: 3, name: 'Vermittlung', desc: 'Routing & Adressierung', protocols: ['IP', 'ICMP', 'IPSec', 'BGP'], pdu: 'Pakete', hardware: 'Router', tcp: 'Internet' },
    { id: 2, name: 'Sicherung', desc: 'Fehlererkennung & Zugriff', protocols: ['Ethernet', 'LLC', 'MAC', 'VLAN'], pdu: 'Frames', hardware: 'Switch / Bridge', tcp: 'Netzzugriff' },
    { id: 1, name: 'Bitübertragung', desc: 'Physikalische Signale', protocols: ['DSL', '1000Base-T', 'ISDN', 'WiFi'], pdu: 'Bits', hardware: 'Hub / Repeater / Kabel', tcp: 'Netzzugriff' },
  ];
  ...
  const EXERCISES = [
    {
      ...
      description: 'Ordne die Protokolle und Hardware-Komponenten den 7 OSI-Schichten zu.',
      type: 'drag-drop-mixed'
    },
    {
      title: 'PDU-Expertencheck',
      description: 'Auf welcher Schicht werden Daten zu Frames, Paketen oder Segmenten?',
      type: 'drag-drop-pdu'
    },
    {
      title: 'Kapselungs-Reihenfolge',
      description: 'Bringe die Schichten in die korrekte Reihenfolge für den Sendeprozess (Top-Down).',
      type: 'sort-layers'
    },
    {
      title: 'TCP/IP 4-Schichten-Modell',
      description: 'Ordne Protokolle und Hardware den 4 Schichten des TCP/IP-Referenzmodells zu.',
  ...
  const QUIZ_QUESTIONS = [
    {
      ...
      correct: 1,
      explain: 'Die Vermittlungsschicht (Layer 3) ist für die Wegfindung und logische Adressierung (IP) zuständig.'
    },
    {
    ...
    },
    {
      ...
      correct: 2,
      explain: 'Das TCP/IP-Modell fasst Sitzungs-, Darstellungs- und Anwendungsschicht des OSI-Modells zusammen.'
    },
    {
      q: 'Auf welcher Schicht arbeitet ein Access Point oder ein Hub?',
      options: ['Schicht 1', 'Schicht 2', 'Schicht 3', 'Schicht 4'],
      correct: 0,
      explain: 'Diese Geräte arbeiten rein auf der physikalischen Ebene (Bitübertragung).'
    }
  ];
  ...
  function renderCurrentTab(root) {
    ...
    switch(currentTab) {
      ...
      case 'exercise': renderExerciseLayout(content); break;
  ...
  function renderExplanation(container) {
    ...
    container.innerHTML = `
      <div class="module-explanation view-enter">
        ...
        <div class="osi-explanation-stack" style="margin-top: var(--space-6);">
          <div class="osi-stack-group-label">Anwendungsschichten</div>
          ${OSI_LAYERS.filter(l => l.id >= 5).map(renderLayer).join('')}
  ...
  function renderExerciseLayout(container) {
    ...
    const hPrev = () => { if (currentExerciseIdx > 0) { currentExerciseIdx--; renderExerciseLayout(container); } };
    const hNext = () => { if (currentExerciseIdx < EXERCISES.length - 1) { currentExerciseIdx++; renderExerciseLayout(container); } };
    
    prev?.addEventListener('click', hPrev);
    next?.addEventListener('click', hNext);
    sub_cleanup_fns.push(() => { prev?.removeEventListener('click', hPrev); next?.removeEventListener('click', hNext); });
  ...
  function setupDragDropListeners(container, total, mode) {
    let score = 0;
    const feedback = container.closest('.module-exercise-card').querySelector('#osiExFeedback');
    ...
    container.querySelectorAll('.osi-drag-item').forEach(d => {
      ...
      sub_cleanup_fns.push(() => { d.removeEventListener('dragstart', onStart); d.removeEventListener('dragend', onEnd); });
    });

    container.querySelectorAll('.osi-drop-zone').forEach(zone => {
      const onOver = (e) => { e.preventDefault(); zone.classList.add('hover'); };
      ...
      const onDrop = (e) => {
        ...
        if (isCorrect) {
          if (mode === 'osi') {
            const lData = OSI_LAYERS.find(l => l.id === parseInt(draggingEl.dataset.layer, 10));
            zone.querySelector('.osi-zone-label').innerText = `L${lData.id}: ${lData.name}`;
          }
          ...
          if (score === total) showFeedbackLocal(feedback, true, 'Hervorragend gelöst!');
        } else {
  ...
  function renderPDUExercise(container) {
    const items = OSI_LAYERS.map(l => ({ text: l.pdu, layer: l.id })).sort(() => 0.5 - Math.random());
    container.innerHTML = `
  ...
  function renderSortExercise(container) {
    ...
    const hCheck = () => {
      const order = [...list.querySelectorAll('.sortable-node')].map(n => parseInt(n.dataset.layer, 10));
      ...
      if (isCorrect) {
        list.querySelectorAll('.sortable-node').forEach(n => {
          const l = OSI_LAYERS.find(x => x.id === parseInt(n.dataset.layer, 10));
          n.innerText = `${l.id}: ${l.name}`;
          n.classList.add('correct');
        });
      }
      showFeedbackLocal(feedback, isCorrect, isCorrect ? 'Korrekt!' : 'Falsche Reihenfolge.');
    };
    checkBtn.addEventListener('click', hCheck);
    sub_cleanup_fns.push(() => { list.removeEventListener('dragover', hOver); checkBtn.removeEventListener('click', hCheck); });
  }
  ...
  function renderQuiz(container) {
    ...
    container.innerHTML = `
      <div class="module-quiz view-enter">
        <div class="module-quiz-header" style="margin-bottom: var(--space-6);">
          <div class="module-quiz-progress"><div class="module-quiz-progress-bar"><div class="module-quiz-progress-fill" style="width: 0%"></div></div></div>
    ...
    container.querySelectorAll('.module-quiz-option').forEach(opt => {
      const h = () => {
        ...
        card.classList.add('answered'); quizAnswered++;
        const qi = parseInt(opt.dataset.qi, 10); const oi = parseInt(opt.dataset.oi, 10);
        const isCorrect = oi === QUIZ_QUESTIONS[qi].correct;
        if (isCorrect) quizScore++;
        opt.classList.add(isCorrect ? 'correct' : 'wrong');
        if (!isCorrect) card.querySelectorAll('.module-quiz-option')[QUIZ_QUESTIONS[qi].correct].classList.add('correct');
        const expl = card.querySelector('.module-quiz-explanation');
        expl.style.display = 'block'; expl.innerHTML = `<strong>${isCorrect ? 'Korrekt!' : 'Falsch.'}</strong> ${QUIZ_QUESTIONS[qi].explain}`;
        container.querySelector('.module-quiz-progress-fill').style.width = `${(quizAnswered / QUIZ_QUESTIONS.length) * 100}%`;
        container.querySelector('.module-quiz-score').innerText = `${quizScore} / ${QUIZ_QUESTIONS.length}`;
  ...
  function showQuizResult(root) {
    ...
    sub_cleanup_fns.push(() => target.querySelector('#btnRestartOsiQuiz')?.removeEventListener('click', h));
  }
  ...
  function getDragAfterElement(container, y) {
    const draggables = [...container.querySelectorAll('.sortable-node:not(.dragging)')];
  ...
  function subCleanup() { sub_cleanup_fns.forEach(fn => { fn(); }); sub_cleanup_fns = []; }
  function cleanup() { subCleanup(); cleanup_fns.forEach(fn => { fn(); }); }
