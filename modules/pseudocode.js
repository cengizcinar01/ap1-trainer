...
const PseudocodeView = (() => {
  ...
  const PUZZLES = [
    {
      ...
      difficulty: 'Leicht',
      description:
        'Ordne die Zeilen so, dass der Rabatt korrekt berechnet wird.',
      correctOrder: [
        'rabatt = 0',
    ...
    },
    {
      ...
      difficulty: 'Mittel',
      description:
        'Sortiere die Zeilen für den inneren Tausch-Logik eines Bubble Sorts.',
      correctOrder: [
    ...
    },
    {
      ...
      difficulty: 'Leicht',
      description:
        'Bringe die Schleife zur Berechnung der Summe von 1 bis n in die richtige Reihenfolge.',
      correctOrder: [
        'summe = 0',
        'i = 1',
        'SOLANGE i <= n MACHE',
  ...
  const ERRORS = [
    {
      ...
      errorLineIndex: 3,
      explanation:
        'Die Variable x wird verringert statt erhöht. Dadurch bleibt x immer kleiner als 5 und die Schleife endet nie.',
    },
    {
      id: 2,
      title: 'Array Index Out of Bounds',
      description:
        'Welche Zeile verursacht einen Fehler beim Zugriff auf das Array (Länge 5)?',
      code: [
      ...
      errorLineIndex: 6,
      explanation:
        'Array-Indizes beginnen meist bei 0. Ein Array der Länge 5 hat die Indizes 0 bis 4. Der Zugriff auf liste[5] ist ungültig.',
    },
    {
      id: 3,
      title: 'Logischer Fehler im Rabatt',
      description:
        'Der Kunde soll ab 100€ einen Rabatt bekommen. Wo ist der Logikfehler?',
      code: [
        'bestellwert = 120',
        'rabatt = 0',
      ...
      errorLineIndex: 2,
      explanation:
        'Die Bedingung "bestellwert < 100" ist falsch. Es soll Rabatt geben, wenn der Wert GRÖSSER oder GLEICH 100 ist.',
    },
  ];
  ...
  function renderContent() {
    ...
    switch (currentTab) {
      case 'puzzle':
        renderPuzzle(contentEl);
        break;
      case 'error':
        renderErrorFinder(contentEl);
        break;
      case 'trace':
        renderTraceTable(contentEl);
        break;
    }
  }
  ...
  function renderPuzzle(container) {
    ...
    document
      .getElementById('btnCheckPuzzle')
      .addEventListener('click', () => checkPuzzle(puzzle));
  ...
  function setupDragAndDrop() {
    ...
    const items = list.querySelectorAll('.code-line');
  ...
  function handleDragEnd(_e) {
    ...
    const list = document.getElementById('puzzleList');
    list.querySelectorAll('.code-line').forEach((item) => {
  ...
  function checkPuzzle(puzzle) {
    const list = document.getElementById('puzzleList');
    const lines = Array.from(list.querySelectorAll('.code-line')).map(
      (el) => el.querySelector('.code-line-content').innerText
    );
    const feedback = document.getElementById('puzzleFeedback');
    const nextBtn = document.getElementById('btnNextPuzzle');
    const checkBtn = document.getElementById('btnCheckPuzzle');

    const isCorrect = lines.every(
      (line, i) => line.trim() === puzzle.correctOrder[i].trim()
    );
    ...
    if (isCorrect) {
      ...
      feedback.innerHTML =
        '<strong>Richtig!</strong> Der Algorithmus ist korrekt.';
      checkBtn.style.display = 'none';
      if (nextBtn) nextBtn.style.display = 'inline-block';
    } else {
      feedback.className = 'module-feedback module-feedback-error';
      feedback.innerHTML =
        '<strong>Nicht ganz...</strong> Die Reihenfolge stimmt noch nicht.';
    }
  }
  ...
  function renderTraceTable(container) {
    ...
    document
      .getElementById('btnCheckTrace')
      .addEventListener('click', () => checkTrace(trace));
    document
      .getElementById('btnFillTrace')
      .addEventListener('click', () => fillTrace(trace));
  ...
  function checkTrace(trace) {
    ...
    if (allCorrect) {
      feedback.className = 'module-feedback module-feedback-success';
      feedback.innerHTML =
        '<strong>Perfekt!</strong> Tabelle korrekt ausgefüllt.';
      document.getElementById('btnCheckTrace').style.display = 'none';
      if (nextBtn) nextBtn.style.display = 'inline-block';
    } else {
      feedback.className = 'module-feedback module-feedback-error';
      feedback.innerHTML =
        '<strong>Fehler gefunden.</strong> Überprüfe die rot markierten Felder.';
    }
  }
